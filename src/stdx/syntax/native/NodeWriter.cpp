...
{
    auto begin =
        NodeFormat::Position(range.begin.fileID, range.begin.line, range.begin.column);
    auto end =
        NodeFormat::Position(range.end.fileID, range.end.line, range.end.column);

    return NodeFormat::CreateRange(builder, &begin, &end);
}
...
flatbuffers::Offset<NodeFormat::DiagInfo> SerializeDiag(const DiagnosticInfo& diag, flatbuffers::FlatBufferBuilder& builder)
{
    ...
    return NodeFormat::CreateDiagInfo(builder, severity, range, msg, hint);
}
...
{
    std::vector<flatbuffers::Offset<NodeFormat::DiagInfo>> diagsVec;
...
flatbuffers::Offset<NodeFormat::File> NodeWriter::SerializeFile(AstFile file, SourceManager* sm)
{
    if (file == nullptr) {
...
flatbuffers::Offset<NodeFormat::MacroDecl> NodeWriter::SerializeMacroDecl(const MacroDecl* macroDecl)
{
    ...
    return NodeFormat::CreateMacroDecl(builder, fbDeclBase, fbFuncBody);
}
...
flatbuffers::Offset<NodeFormat::Annotation> NodeWriter::SerializeAnnotation(AstAnnotation annotation)
{
    ...
    return NodeFormat::CreateAnnotation(builder, base, kind, &identPos, identifier, fbArgs,
