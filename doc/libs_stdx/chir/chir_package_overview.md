...
| [GlobalValue](./chir_package_api/chir_package_classes.md) | 全局值的密封抽象基类。 |
...
| [Package](./chir_package_api/chir_package_classes.md#class-package) | CHIR 包，提供全局变量（`globalVars`）、函数（`functions`）、全部自定义类型定义（`allCustomTypeDefs`）、包初始化函数（`packageInitFunc`）等查询接口，以及添加函数（`addFunction`）、添加全局变量（`addGlobalVar`）、按签名获取函数（`getSpecifiedFunction`）等构建接口。 |
...
| 函数名 | 功能 |
| --- | --- |
| [serializePackage](./chir_package_api/chir_package_classes.md#func-serializepackage) | 将 Package 序列化为二进制数据。 |
| [deserializePackage](./chir_package_api/chir_package_classes.md#func-deserializepackage) | 从二进制数据反序列化 Package。 |
