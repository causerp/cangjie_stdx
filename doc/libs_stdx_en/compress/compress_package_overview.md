...
Compression refers to compressing/decompressing a single data stream or byte buffer, implementing various compression formats (e.g., deflate, gzip, etc.). This capability is provided by the submodule stdx.compress.zlib.
...
Compression and archiving are typically used in combination, with a common practice being to archive first and then compress (e.g., tar.gz). This package provides convenient combination tools at the top level (e.g., TarGzip), which combine packaging and compression into a single-step read/write interface for common scenarios.
